...
@role('admin')
    <div class="d-flex flex-row justify-content-between align-items-center">
        ...
        <span class="icheck-{{setting('theme_color')}}">
            @php
                $kycStatus = isset($eProvider) ? ($eProvider->kyc_status ?? 'pending') : 'pending';
                $kycBlocked = isset($eProvider) && $eProvider->id && !in_array($kycStatus, ['approved', 'verified']);
            @endphp
            @if($kycBlocked)
                {!! Form::checkbox('accepted', 1, null, ['disabled' => 'disabled', 'title' => 'KYC verification required before approval']) !!} <label for="accepted"></label>
            @else
                {!! Form::checkbox('accepted', 1, null) !!} <label for="accepted"></label>
            @endif
        </span>
    </div>
    @if($kycBlocked)
        <div class="mx-3">
            <small class="text-danger">
                ...
                Cannot approve: KYC status is <strong>{{ ucfirst($kycStatus) }}</strong>. 
                Vendor must complete KYC verification before they can be accepted.
                @if(isset($eProvider) && $eProvider->id)
                    <a href="{{ url('admin/kyc/' . $eProvider->id) }}" class="text-primary ml-1">View KYC →</a>
                @endif
            </small>
        </div>
    @endif
