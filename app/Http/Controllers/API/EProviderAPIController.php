...
class EProviderAPIController extends Controller
{
    ...
    public function store(CreateEProviderRequest $request): JsonResponse
    {
        try {
            $input = $request->all();
            // Extract image UUIDs before passing to repository — images are handled via Spatie Media Library
            $imageUuids = $input['image'] ?? [];
            unset($input['image']);
            ...
            if ($imageUuids && is_array($imageUuids)) {
                foreach ($imageUuids as $fileUuid) {
