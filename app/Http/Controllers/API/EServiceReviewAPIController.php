...
class EServiceReviewAPIController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $eServiceReviews = $this->eServiceReviewRepository->with(['user'])->all();
    ...
    {
        // Security: the reviewer is always the authenticated user — never trust a
        // user_id supplied in the request (otherwise a vendor could forge reviews
        // for arbitrary users / inflate their own ratings).
        $userId = auth()->id();
        if (!$userId) {
            return $this->sendError(__('auth.failed') ?: 'Unauthenticated', 401);
        }
        $uniqueInput = ['user_id' => $userId, 'e_service_id' => $request->input('e_service_id')];
        $otherInput = $request->except('user_id', 'e_service_id', 'api_token');
        try {
