...
class EServiceAPIController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        if ($request->has('search')) {
            $request->merge(['searchFields' => 'name:like;description:like']);
        }
        try {
    ...
    public function store(CreateEServiceRequest $request): JsonResponse
    {
        try {
            ...
            $user = auth()->user();
            $providerId = $input['e_provider_id'] ?? null;
            if (!$providerId && $user) {
                $provider = $user->eProviders()->first();
                $providerId = $provider ? $provider->id : null;
            }

            // Enforce subscription package service limit (max_services)
            if ($providerId && class_exists('\Nwidart\Modules\Facades\Module') && \Nwidart\Modules\Facades\Module::isActivated('Subscription')) {
                $activeSub = \Modules\Subscription\Models\EProviderSubscription::where('e_provider_id', $providerId)
                    ->valid()
                    ->with('subscriptionPackage')
                    ->first();

                $maxServices = $activeSub && $activeSub->subscriptionPackage
                    ? (int) $activeSub->subscriptionPackage->max_services
                    : 5; // Essential & Free Trial limit

                if ($maxServices !== -1) {
                    $currentServiceCount = $this->eServiceRepository->findWhere(['e_provider_id' => $providerId])->count();
                    if ($currentServiceCount >= $maxServices) {
                        ...
                        return $this->sendError("You have reached your limit of {$maxServices} services ({$tierName} tier). Please upgrade your subscription to list more services.");
                    }
                }
            }
