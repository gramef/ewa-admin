    ...
    public function wallets(string $walletId, Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($wallet->currency->code == setting('default_currency_code')) {
                // Wallet payments must reference the Wallet payment method, otherwise the
                // payments.payment_method_id FK fails. (cash() sets this; wallets() didn't.)
                $walletMethod = $this->paymentMethodRepository->findWhere(['route' => '/Wallet'])->first();
                if (!$walletMethod) {
                    return $this->sendError(__('lang.not_found', ['operator' => __('lang.payment_method')]) . ' Wallet');
                }
                $input['payment']['amount'] = $booking->getTotal();
                $input['payment']['description'] = __('lang.payment_booking_id') . $input['id'];
                $input['payment']['payment_status_id'] = 2; // done
                $input['payment']['payment_method_id'] = $walletMethod->id;
                ...
                // Atomic: the debit (which validates balance via WalletTransactionCreatingEvent),
                // the payment record and the booking link must all succeed or all roll back —
                // previously a failed payment insert left an orphan debit transaction.
                $payment = \DB::transaction(function () use ($transaction, $input) {
                    $this->walletTransactionRepository->create($transaction);
                    $payment = $this->paymentRepository->create($input['payment']);
                    $this->bookingRepository->update(['payment_id' => $payment->id], $input['id']);
                    return $payment;
                });
