...
class UserAPIController extends Controller
{
    ...
    function login(Request $request)
    {
        try {
            ...
            if (auth()->attempt(['email' => $request->input('email'), 'password' => $request->input('password')])) {
                ...
                $this->userRepository->update(['device_token' => $user->device_token], $user->id);
    ...
    function register(Request $request)
    {
        try {
            ...
            // Ensure 'provider' role is always assigned for vendor registrations
            if (!in_array('provider', $defaultRoles)) {
    ...
    function settings(Request $request)
    {
        ...
        // Get enabled modules
        $moduleController = new \App\Http\Controllers\API\ModuleAPIController();
        $modulesResponse = $moduleController->index(request());
        $modulesData = json_decode($modulesResponse->getContent(), true);
        $settings['modules'] = $modulesData['success'] ? $modulesData['data'] : [];
