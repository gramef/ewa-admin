    ...
    public function create()
    {
        ...
        foreach ($eProviders as $eProvider) {
            try {
                ...
                $payout = $this->eProviderPayoutRepository->where('e_provider_id', $eProvider->id)->where('paid', '1')->sum('amount');
                $this->earningRepository->updateOrCreate(['e_provider_id' => $eProvider->id], [
                        ...
                        'e_provider_earning' => (($total - $tax ) * $eProvider->eProviderType->commission / 100) - $payout,
                        'payout' => $payout,
                    ]
                );
