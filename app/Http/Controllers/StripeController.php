...
class StripeController extends ParentBookingController
{
    ...
    public function createPaymentIntent(Request $request)
    {
        try {
            ...
            Log::info('createPaymentIntent called', ['booking_id' => $bookingId]);
            ...
            if (empty($booking)) {
                return response()->json(['error' => 'Booking not found: ' . $bookingId], 404);
            }

            $stripeSecret = env('STRIPE_SECRET') ?: setting('stripe_secret') ?: config('services.stripe.secret');
            $stripeSecret = !empty($stripeSecret) ? trim($stripeSecret) : null;
            if (empty($stripeSecret)) {
                Log::error('Stripe secret is missing from both setting() and .env');
                return response()->json(['error' => 'Stripe not configured. Please set stripe_secret in admin or STRIPE_SECRET in .env'], 500);
            }

            Stripe::setApiKey($stripeSecret);

            $amount = (int)($booking->getTotal() * 100);
            ...
            $paymentIntent = PaymentIntent::create([
                ...
                'description' => 'EWA Booking #' . $booking->id,
            ]);

            return response()->json(['clientSecret' => $paymentIntent->client_secret]);
        } catch (\Exception $e) {
            Log::error('createPaymentIntent error', [
